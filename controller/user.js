...
'compare' de "bcrypt" pour comparer le mots de passe hashé enregistré dans la BDD avec celui que l'utilisateur a entré. 
Si le resultats est positif (true), renvoie un objet contenant l''_id' de l'utilisateur ainsi qu'un token sur la base de l'id
    ...
    .then(user => {
      ...
      bcrypt
        .compare(
          req.body.password,
          user.password
        ) /* Sinon, comparaison du hash de la BDD avec le MDP entré*/
        .then(valid => {
          if (!valid) {
